...
import CreateDegreeForm from "./CreateDegreeFrom";
...
export default function DegreeList() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = degreeList.map((n) => n.id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - degreeList.length) : 0;

  return (
    <>
      <PageHeader
        title="Danh sách bằng cấp"
        action={<CreateDegreeForm triggerReFetch={triggerReFetch} />}
      />
              ...
              <Controller
                ...
                render={({ field }) => (
                  <TextField
                    {...field}
                    fullWidth
                    placeholder="Tìm kiếm theo tên bằng cấp"
                    InputProps={{
                      startAdornment: (
