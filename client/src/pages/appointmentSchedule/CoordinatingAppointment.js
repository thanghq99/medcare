...
function CoordinatingAppointment({ appointment, triggerReFetch }) {
  const { user } = useAuth();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      console.log("assigned doctor to appointment", result);
      toast.success("Đã điều phối bác sĩ cho lịch hẹn!");
      triggerReFetch();
      handleClose();
    } catch (error) {
      console.log("cant assign doctor to appointment", error);
  ...
  if (user.isAdmin === true && appointment.status === "requested")
    return (
      <>
...
export default CoordinatingAppointment;
