...
import CreateSpecialtyForm from "./CreateSpecialtyFrom";
...
export default function SpecialtyList() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = specialtyList.map((n) => n.id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - specialtyList.length) : 0;

  return (
    <>
      <PageHeader
        title="Danh sách chuyên khoa"
        action={<CreateSpecialtyForm triggerReFetch={triggerReFetch} />}
      />
