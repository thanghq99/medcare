...
import CreateSubclinicalForm from "./CreateSubclinicalFrom";
...
export default function SubclinicalList() {
  const [selected, setSelected] = React.useState([]);
  const [page, setPage] = React.useState(0);
  const [dense, setDense] = React.useState(false);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);
  const [subclinicalList, setSubclinicalList] = React.useState([]);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = subclinicalList.map((n) => n.id);
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - subclinicalList.length)
      : 0;
  ...
  return (
    <>
      <PageHeader
        ...
        action={<CreateSubclinicalForm triggerReFetch={triggerReFetch} />}
      />
              ...
              <Controller
                ...
                render={({ field }) => (
                  <TextField
                    {...field}
                    fullWidth
                    placeholder="Tìm kiếm theo tên Cận lâm sàng"
                    InputProps={{
                      startAdornment: (
        ...
        <Paper sx={{ width: "100%", mb: 2 }}>
          <TableContainer component={Paper} sx={{ position: "relative" }}>
            <TableHeadReplacement
              ...
              rowCount={subclinicalList.length}
            />
            <Table
              sx={{ minWidth: 750 }}
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <ListItemAction
                            ...
                            subclinicalList={subclinicalList}
                          />
                        </TableCell>
                      </TableRow>
                    );
